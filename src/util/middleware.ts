...
function calculateTotalElements(
  elementsCounter: Record<string, number>,
): number {
...
function parseEvaluation(evaluation: any): any {
  ...
  report['data'].elems = structuredClone(elements);
  report['data'].nodes = structuredClone(nodes);
  ...
  report['data'].tot.info.url = structuredClone(report['data'].rawUrl);
  report['data'].tot.info.title = structuredClone(report['data'].title);
  report['data'].tot.info.date = structuredClone(report['data'].date);
  report['data'].tot.info.htmlTags = calculateTotalElements(
    evaluation.modules.counter.data.tags,
  );
  ...
  report['data'].tot.elems = structuredClone(report['data'].elems);
  report['data'].tot.results = structuredClone(results);
  report['data'].conform = calculateConform(report['data'].tot.results);
  report['data'].tot.info.conform = structuredClone(report['data'].conform);

  completeMissingReportElements(report);

  report['data'].score = generateScore(report);
  report['data'].tot.info.score = structuredClone(report['data'].score);
  return report;
}
